package eu.atos.sla.tnova.parser.json;
...
import eu.atos.sla.tnova.parser.data.wsag.AllTerms;
import eu.atos.sla.tnova.parser.data.wsag.ServiceDescriptionTerm;
import eu.atos.sla.tnova.parser.data.wsag.Template;
import eu.atos.sla.tnova.parser.data.wsag.Terms;
/**
 * 
...
public class TemplateParser implements IParser<ITemplate> {
	...
	public ITemplate getWsagObject(String serializedData) throws ParserException{
		...
		try{
			...
			template = (ITemplate)mapper.readValue(serializedData, Template.class);
			
			logger.debug("Template parsed {}", template);
			System.out.println(template.getTerms().getAllTerms().getServiceDescriptionTerm().getClass());
			System.out.println(((ServiceDescriptionTerm)template.getTerms().getAllTerms().getServiceDescriptionTerm()).getRequirements().get(0).getName());
	...
	public String getWsagAsSerializedData(String serializedData) throws ParserException {
		...
		try {
			ITemplate template = getWsagObject(serializedData);
