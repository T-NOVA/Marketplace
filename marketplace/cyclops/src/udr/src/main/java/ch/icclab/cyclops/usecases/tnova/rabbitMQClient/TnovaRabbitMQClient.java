...
public class TnovaRabbitMQClient implements Runnable {
    final static Logger logger = LogManager.getLogger(TnovaRabbitMQClient.class.getName());
     ...
     */
    public TnovaRabbitMQClient(String queueName) {
     ...
     */
    private TnovaEvent processMessage(String message) {
        Gson gson = new Gson();

        TnovaEvent data = gson.fromJson(message, TnovaEvent.class);
